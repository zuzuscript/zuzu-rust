...
pub(super) fn call_object_method(
    ...
    Some(match name {
        ...
        "binv" => Ok(make_bignum_auto(
            BigDecimal::from_f64(1.0 / to_f64(&value)).unwrap_or_default(),
        )),
        "bsin" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).sin()).unwrap_or_default(),
        )),
        "bcos" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).cos()).unwrap_or_default(),
        )),
        "btan" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).tan()).unwrap_or_default(),
        )),
        "bsqrt" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).sqrt()).unwrap_or_default(),
        )),
        "bround" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).round()).unwrap_or_default(),
        )),
        "bfloor" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).floor()).unwrap_or_default(),
        )),
        "bceil" => Ok(make_bignum_auto(
            BigDecimal::from_f64(to_f64(&value).ceil()).unwrap_or_default(),
        )),
    ...
    match value {
        ...
        other => {
            ...
            BigDecimal::from_f64(number)
                .ok_or_else(|| ZuzuRustError::runtime("cannot represent Number as BigNum"))
        }
    }
}
...
fn to_hex(value: &BigDecimal, text: &str) -> String {
    ...
    let integer = decimal
        .split_once('.')
        .map_or(decimal.as_str(), |(integer, _)| integer);
    let parsed = BigInt::from_str(integer)
        .or_else(|_| BigInt::from_str(text))
        .unwrap_or_else(|_| BigInt::zero());
    if parsed.is_zero() {
        "0x0".to_owned()
    } else if parsed < BigInt::zero() {
        format!("-0x{}", (-parsed).to_str_radix(16))
    } else {
        format!("0x{}", parsed.to_str_radix(16))
    }
}
...
fn pow_bignum(left: BigDecimal, left_text: &str, is_left_integer: bool, args: &[Value]) -> Value {
    let right = args
        ...
        .map(coerce_other)
        .transpose()
        .unwrap_or_else(|_| Some(BigDecimal::zero()))
        .unwrap_or_else(BigDecimal::zero);
